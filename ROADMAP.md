...
**目标**：用户不用每天主动打开，被触发过的信号自己会留在站内等他看。

**范围决策**：先只做**站内提醒**，外部推送（Server酱/钉钉/邮件/Webhook）后置到 V6.5。
理由：先验证「规则引擎判得准不准」和「用户是否真的需要」，再决定要不要为推送付架构成本。
