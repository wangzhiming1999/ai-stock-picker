...
def compute_signals(
    ...
    # 支撑/压力的筛选判据**两侧不同**，这是有意设计：
    #
    # 压力侧用**现价**：旧实现 `r > reference`（reference = 最后一根已完成 K 线的收盘）
    # 在趋势行情里会分叉 —— 上行时现价已涨过 reference，一个通过 reference 检验的候选
    # 可能仍**低于现价**，被 min() 选中后产出「压力位在现价下方」的自相矛盾档位。
    # 实测（120 根，各形态 200 次）：阶梯上行 / 平滑上行 / 区间上沿 → 压力位距现价
    # p10 = −0.16% ~ −0.27%，rr_ratio 中位 0.07~0.10、100% < 0.5。
    # 因此压力取「高于现价 + 最小距离」（_MIN_RESISTANCE_GAP_PCT）中最近者。
    #
    # 支撑侧用**已收盘K的收盘 basis[-1]**：支撑是「此前已存在的结构位」，
    # 现价跌破它正是「跌破支撑」sell 信号的定义。若改用 `s < current` 筛选，
    # 跌破发生后支撑会被重选到更低的候选（如 ma20），跌破在定义上永不触发
    # （实测 closes=[30,50]+[35]*38+[39.8]*20、price=39：旧逻辑 sell「跌破支撑」，
    # 现价判据下 hold「持有观察」—— 盯盘失去核心预警）。
    # 「支撑 ≤ basis[-1]」保证候选在昨日已成立；现价与它的关系由 monitor._advice 判定。
    min_res_gap = current * _MIN_RESISTANCE_GAP_PCT

    # 主支撑：昨日已成立（≤ basis[-1]）的候选里取最高者（最近的下方结构）
    supports = [fib_618, bb_lower, ma20, ma60]
    valid_supports = [s for s in supports if s <= basis[-1]]
