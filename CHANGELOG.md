...
> 代码仓库版本见 `backend/app/main.py` 的 `version` 字段，**当前为 `0.2.0`，对应产品里程碑 V1–V5.28**。
>
...
- **失败可见性补强 + 默认技巧集合 KeyError 修复**（2026-09-21）
  ...
  > 一处事故级 KeyError（默认技巧集合含已下线的 `ma20_slope`，形态回测/扫描/体检
  > 三个入口不传技巧时恒定 502，且形态回测是技巧升 verified 的唯一通道）。
  - `pattern_service.ACTIVE_TACTICS`：默认集合统一 = `TACTIC_MAP` 的键，3 处入口改用；
    守卫 `test_tactic_key_sets.py` 钉不变量并**反向验证判别力**
  - 历史换手率口径修正：日/周/月线**都有值**（旧注释「仅日线」是错的）；回测里
    「换手率 >30%」成为生效条件，个别缺值按实报数（`3/10 个时点缺…`），`volume_peak`
    证据档标注旧数字出自旧口径、待重跑
  - 回测基准失败附 `benchmark_note`（前端展示）；复盘块失败说明并进 `summary`；
    回测异常时点计数报出；`volume_floor` 标签改常量插值（修「28% 打 ✓ 标 ≤20%」自相矛盾）；
    `CaliberLine` 只在有结果时渲染
  - 守卫 `test_failure_visibility.py` 4 条；后端 634 + 41 subtests passed
...
- **决策可用性：把「打板什么、什么价」补上 + 预期价格贯穿四个环节**（2026-09-21）
  > 用户反馈三连：①「这个可打板，但是没说打板什么」；②「决策类文案太少了，我都不知道怎么操作」；
  > ③「选股的时候记的时候记得加一个预期价格」。三句话同一个缺陷 —— **一直在给判断，却没给落点**。
  > 判断对不对是统计问题，有没有落点是可用性问题，后者不需要等回测就能修。
  > 本轮只补落点，**不动任何证据档**（`ACTIONABLE_TIERS` 仍只有 `verified`，今仍 0 条）。
  - **「可打板」档补候选清单**（`limitup_service.build_focus`）：每只给**打板价（今日涨停价）**
    与**次日涨停价**（交易所涨跌幅规则算的算术，不是预测），加换手 / 封单比 / 首封 / 溢价读数
    与明日晋级读数，每条 `basis` 都带 n。涨停幅度按交易板分（主板 10 / 创业板·科创板 20 /
    北交所 30 / **主板 ST 5**，判定顺序即优先级 —— 创业板 ST 仍是 20%）；次日涨停价用
    `ROUND_HALF_UP`，因为 Python 内置 `round()` 是银行家舍入，10.05×1.1=11.055 会给 11.05
    而交易所进位 11.06 —— **差一分钱就挂不上单**。
  - **连板与首板分两组，口径不可比、不相加**：晋级读数（44.6~44.8%）是 n=164 的**连板**样本，
    首板引用它等于把口径套到没测过的样本上。首板只引溢价与封板时间档，不借连板的数字。
  - **候选硬筛只用已回测的负期望边界**：换手 <5%（缩量一字/秒板挂不上单）、尾盘封板
    （唯一负期望档）、炸板 ≥3 次（期望降至 +0.68%）。剔除计数按「首个命中原因」归类，
    保证 `各档计数之和 + 入选数 ≡ 涨停总数`（有测试守着这个恒等式）。
  - **环境不过关就不给清单**：`watch` / `avoid` 两档 `focus=None` + 一句说明为什么没有 ——
    说「别动手」的同时列一张可买名单，是本项目在观察层明确踩过的自相矛盾。
    `hunt` 档另给 `playbook` 执行步骤；**刻意不给仓位百分比**（没有任何回测支持具体比例，
    编一个出来就是假精确）。
  - **预期价格 = 执行锚点**（统一口径 `signal_service.expected_price_from_levels`），
    贯穿四处：① 涨停带打板候选（锚点 = 涨停价）；② 每日推荐卡片（回踩位 / 突破位 + 距现价 ±x%）；
    ③ 决策先锋三维榜与诊股（`auto` 档：现价 ≥ 主压力 ×0.98 → 突破位，否则回踩位；
    边界取 0.98 而非 1.00，否则同一只票会在两次取数间跳档）；④ 模拟盘记账（建仓可填计划价，
    流水显示「成交比预期贵/便宜 x%」）。同一行的**触发文案与预期价格必须取同一个数**，
    否则会出现「触发写 12.30、价格给 12.85」这种自相矛盾（有测试钉死）。
  - **口径隔离三条硬约束**：预期价格（a）不参与**任何**收益率与胜率结算，只作挂单价参考；
    （b）取自 `monitor_levels` 的结构位，而**结构位买入侧实测为负**，四处 UI 都写明这点，
    不得因此把 `buy_point` 说成买点；（c）拿不到就 `None` + UI 显示「—」，**永不折算成现价**
    （折算出来的假锚点会被当成真价位用）。
  - 迁移 `supabase-schema-v13.sql`：`daily_recommendations.expected_price` /
    `sim_trades.expected_price`（幂等 `ADD COLUMN IF NOT EXISTS`）。**两处都做了降级**：
    推荐落库在列缺失时剥掉该列重试一次（丢它会让整条推荐落库失败 → 连带丢胜率跟踪）；
    模拟盘在列缺失时静默跳过回填（只丢锚点，不影响现金与成交）。抢跑迁移不会坏任何链路。
  - 模拟盘的预期价格**不引入 RPC 签名变更**：`execute_sim_trade` 加参数会创建**重载**而不是替换，
    线上会出现两个同名函数、调用方还得处理歧义；成交已落库、id 已拿到，补一条 UPDATE 更安全。
  - 推荐快照 `_RECOMMENDATION_SCHEMA_VERSION` 4 → **5**：v4 快照没有预期价格字段，
    而缓存按数据日共享 —— 不升版本就会整整一天给不出预期价格。
  - 前端：折叠态徽标直接带出候选数（「今日建议：可打板 · 候选 3 只」），展开后是候选清单 +
    剔除明细 + 执行步骤；预期价格在三维榜主表就露出、展开给完整说明；模拟盘滑点用
    「贵/便宜」文字而**不是红绿**（滑点是执行成本，不是价格方向）。
  - 后端测试 +35（全量 **615 passed**）、前端 +11（`npm test` **55 passed**）、
    `tsconfig.app.json` typecheck 零错误、`vite build` 通过；`featureMap.ts` 四处登记同步更新。
  - ⚠️ **上线前需要执行 v13 迁移**：不做也不报错，但预期价格**存不下来** ——
    推荐落库会降级跳过该列、模拟盘那笔回填会被跳过（`POST /api/admin/migrate?only=v13`）。
...
### Fixed
- **资金维漏配 `f184`，导致「潜力龙头」永久为空**（2026-09-21，上线当天实测发现）
  > `_EM_FF_FIELDS` 请求字段里少了 `f184`（主力净占比），而 `_fund_flow_row` 会读它 ——
  > 解析器不报错、只是永远拿到 None，于是资金维只剩「净额加成」，**分数被压在 5.0~5.6**
  > （线上 20 只实测 min 5.0 / median 5.3 / max 5.6）。而「潜力龙头」的门槛是资金分 ≥7 →
  > 整块**永远不可能有结果**，界面上只是看起来「今天没有龙头」。这正是本项目反复遇到的
  > 同一类失效：不崩、不报错、只是永远不触发。
  - 字段表补 `f184`，与 akshare `stock_individual_fund_flow_rank` 打**同一个 clist 端点**
    所用的字段串一致（对照其源码确认：`f12,f14,f2,f3,f62,f184,f66,f69,f72,f75,f78,f81,f84,f87,…`）
  - **新增两条守卫测试，锁的正是这次漏掉的那一层**：① 请求字段必须覆盖 `_fund_flow_row`
    源码里读取的每个 `fN`（`inspect` 抽源码比对；旧的 `assert "f62" in fields` 正是这样放过去的，
    已反向验证判别力：对旧字段串精确报出 `f184`）；② 龙头门槛必须够得着 —— 用**解析器产出的行**
    算分并断言 ≥ 门槛，而旧测试全部手写 dict，永远测不到取数侧漏配。
  - 附带把门槛从写死的 `7 / 6 / 9` 提成具名常量（`LEADER_MIN_DARK_MONEY` 等），
    让「门槛值」本身也变成可断言的契约；后端 **580 passed**（+2）
