  ...
  return (
    <span className="relative inline-flex items-center">
      <motion.button
        ...
        className={`inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-meta transition-colors ${
          active
            ? "text-amber-400"
            : "text-ink-muted hover:bg-surface-line/50 hover:text-amber-300"
        }`}
      >
      ...
      {msg && (
        <span className={`absolute left-0 top-full z-10 whitespace-nowrap rounded-md bg-surface-inset px-1.5 py-0.5 text-meta ${msg.includes("已") ? "text-amber-300" : "text-red-300"}`}>
          {msg}
        </span>
      )}
    </span>
