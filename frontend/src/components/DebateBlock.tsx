  ...
  return (
    <div className={`${SUB_QUIET} p-3`}>
      <div className="flex items-center justify-between">
        <span className={`flex items-center gap-1.5 text-meta font-semibold ${isBull ? upTone() : downTone()}`}>
          {isBull ? "多头研究员" : "空头研究员"}
        </span>
        <span className={`text-meta tabular-nums ${TEXT.meta}`}>
          信心 <span className="font-semibold text-ink">{side.confidence.toFixed(0)}</span>
        </span>
      </div>
      <p className={`mt-1.5 text-body leading-relaxed ${TEXT.body}`}>{side.thesis || "—"}</p>
      {side.evidence.length > 0 && (
        <ul className={`mt-2 space-y-1 text-meta leading-relaxed ${TEXT.meta}`}>
      ...
      {side.rebuttal.length > 0 && (
        <div className={`mt-2 border-t border-surface-line-soft pt-2`}>
          <p className="text-meta font-medium text-ink-muted">对对方的反驳</p>
  ...
  return (
    <div className="mt-4 rounded-xl border border-surface-line bg-surface-panel/60 p-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <h4 className="text-body font-semibold text-ink">多空研究员对辩</h4>
        <span className={`rounded-md bg-surface-inset/60 px-1.5 py-0.5 text-meta ${TEXT.meta}`}>
          {debate.rounds} 轮
        </span>
        <span className={`text-meta font-medium tabular-nums ${divergenceTone(debate.divergence)}`}>
          分歧度 {debate.divergence.toFixed(0)} · {divergenceLabel(debate.divergence)}
        </span>
        <span className="ml-auto rounded-md bg-surface-inset/60 px-1.5 py-0.5 text-meta text-ink-muted">
      ...
      {debate.key_disagreement && (
        <p className={`mt-2 text-meta leading-relaxed ${TEXT.meta}`}>
      ...
      {/* 闸门文案：原样展示，不得删改 */}
      {debate.gate_note && (
        <p className={`mt-3 flex items-start gap-1.5 text-meta leading-relaxed ${TEXT.meta}`}>
