...
function SectorCompare({ sector }: { sector: VanguardSector }) {
  return (
    <div className={`${SUB} px-3 py-2 text-xs`}>
...
export default function DiagnoseCard({ code, requestId }: Props) {
  ...
  return (
    <div className="space-y-3">
      ...
      {err && (
        <div className="rounded-lg border border-red-800 bg-red-950/40 px-3 py-2 text-sm text-red-300">{err}</div>
      )}
      ...
      {data && item && (
        <div className="space-y-3">
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
            <span className="text-base font-semibold text-ink-strong">{item.name || data.code}</span>
            <span className="text-xs text-ink-faint">{data.code}</span>
            ...
            {data.date && <span className="text-xs text-ink-faint">{data.date}</span>}
          ...
          <div className="grid gap-3 lg:grid-cols-[1fr_1fr_1.1fr]">
            ...
            <div className={`${SUB} px-3 py-2`}>
              <p className="mb-1.5 text-xs font-semibold text-ink-muted">资金 / 位置</p>
              <div className="space-y-1 text-xs text-ink-soft">
                {fund ? (
                  ...
                  <div className="text-ink-faint">该股不在当日资金流批次内（或资金维不可用）</div>
                )}
                {metrics ? (
                  <>
                    <div>
                  ...
                  <div className="text-ink-faint">K 线不足 60 根，趋势维与结构位不可用</div>
                )}
              </div>
            </div>
          ...
          {data.note && <p className="text-xs text-amber-300/90">{data.note}</p>}
        </div>
      )}
    </div>
  );
