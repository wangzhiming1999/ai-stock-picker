  ...
  return (
    <AnimatePresence>
      {open && (
        ...
        >
          <motion.div
            className={`${CARD_MODAL} w-full max-w-md`}
          ...
          >
            <div className="mb-5 flex items-center justify-between">
              <h2 className="flex items-center gap-2 text-num font-bold text-white">
            ...
            <div className="mb-4 flex rounded-lg border border-surface-line bg-surface-inset/70 p-1">
              {(["signin", "signup"] as const).map((m) => (
                <motion.button
                  ...
                  className={`relative flex-1 rounded-md px-4 py-1.5 text-body font-medium transition-colors ${
            ...
            <label className="mb-3 block">
              <span className="mb-1 flex items-center gap-1.5 text-body text-ink-muted">
              ...
              <Input
                ...
                className="w-full rounded-lg bg-surface-inset/70 px-3 py-2 text-body" />
            </label>
            ...
            {error && (
              <motion.div
                ...
                className="mb-3 rounded-lg border border-state-danger-line bg-state-danger-surface px-3 py-2 text-body text-state-danger-soft"
              >
                {error}
              </motion.div>
            )}
            {notice && (
              <motion.div
                initial={{ opacity: 0, y: -6 }}
                animate={{ opacity: 1, y: 0 }}
                className="mb-3 rounded-lg border border-green-800 bg-green-950/40 px-3 py-2 text-body text-green-300"
              >
                {notice}
              </motion.div>
            )}
            ...
            <motion.button
              ...
              className="w-full rounded-lg bg-brand py-2.5 text-body font-medium text-white hover:bg-brand-dark">
