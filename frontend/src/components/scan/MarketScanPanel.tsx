    ...
    >
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
        <label className="block">
          ...
          <Input value={filters.minChange} onChange={(e) => onFilterChange("minChange", e.target.value)} className="w-full rounded-lg bg-slate-800/70 px-3 py-1.5 text-sm" />
        </label>
        <label className="block">
          <span className="mb-1 block text-xs text-ink-faint">最低成交额(亿)</span>
          <Input value={filters.minAmount} onChange={(e) => onFilterChange("minAmount", e.target.value)} className="w-full rounded-lg bg-slate-800/70 px-3 py-1.5 text-sm" />
        </label>
        <label className="block">
          <span className="mb-1 block text-xs text-ink-faint">最低股价</span>
          <Input value={filters.minPrice} onChange={(e) => onFilterChange("minPrice", e.target.value)} className="w-full rounded-lg bg-slate-800/70 px-3 py-1.5 text-sm" />
        </label>
        <label className="block">
          <span className="mb-1 block text-xs text-ink-faint">最高股价</span>
          <Input value={filters.maxPrice} onChange={(e) => onFilterChange("maxPrice", e.target.value)} className="w-full rounded-lg bg-slate-800/70 px-3 py-1.5 text-sm" />
        </label>
        <label className="block">
          <span className="mb-1 block text-xs text-ink-faint">数量上限</span>
          <Input value={filters.limit} onChange={(e) => onFilterChange("limit", e.target.value)} className="w-full rounded-lg bg-slate-800/70 px-3 py-1.5 text-sm" />
        </label>
      </div>
      <button
        onClick={onScan}
